...
# Table
LISA_tb = paste0("LISA",2012:2017,"_Individ")
background_tb = "Bakgrundsdata"
geo_tb = paste0("Geo",2012:2017)
muni_tb = paste0("la",2012:2017,".txt")
# Column/Variable names
lisa.names = c("PersonLopNr", "Sun2000niva","FamStF",
               "SocBidrpersF04","DispInk04",'SyssStat11',"FamTypF","Civil","ArbLos",
...
lisa.names2 = c("PersonLopNr", "Sun2000niva","FamStF",
               "SocBidrpersF04","DispInk04",'SyssStat11',"FamTypF","Civil","ArbLos",
               "ForPeng","ForLed","SjukPP","BostBidrPersF04","KapInk","Barn0_3","Barn4_6",
               "Barn7_10","Barn11_15","Barn16_17","Lan","Kommun","ArbSokNov","AlosDag","AK14Dag",
               "Ssyk4_2012_J16","PeOrgLopNr","CfarLopNr","StudDelt","AntFlyttTot")
...
for (i in 1:length(LISA_tb)) {
  if (i<3) {
    ...
    lisadata[[i]] =  sqlQuery(myconn, paste('select ', paste(lisa.names2, collapse = ','), 
                                            ' from ', LISA_tb[i], sep = ''))
  }
  ...
  Final_data = Final_data %>% subset(FodelseAr == 1983|FodelseAr == 1982|FodelseAr == 1981|FodelseAr == 1980|FodelseAr == 1979|FodelseAr == 1978|
                                       FodelseAr == 1977|FodelseAr == 1976|FodelseAr == 1975) %>% 
 as_tibble()
...
#  )
  ...
  incomes = paste0("income",2012:2017)
  # Export (just run!)
